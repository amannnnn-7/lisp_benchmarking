...
# Define the target difficulties (you can change these as needed)
# Difficulty levels: EASY (1), MEDIUM (2), HARD (3), HARDER (4), HARDEST (5)
target_difficulties = [0, 1, 2, 3, 4, 5]  # Collect all rows for these difficulties
...
# Dictionary to store collected samples for each difficulty
...
for i, example in enumerate(tqdm(ds_train, desc="Processing dataset")):
    ...
    # Check if the current example's difficulty is one we are looking for
    if difficulty in target_difficulties:
        # Add the example to the list for this difficulty
        collected_samples[difficulty].append(example)

# After the loop, combine all collected examples into a single list
...
if not all_collected_examples:
    ...
    # Convert the list of dictionaries back into a Dataset object
    ...
    try:
        # Use the to_csv method provided by the datasets library
        ...
        # Optional: Verify the file exists and print its size
